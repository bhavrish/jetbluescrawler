...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for connectionsTweet in connectionsTweets[:]:
            if not JetblueAggregateModel.objects.filter(tweet_id=connectionsTweet.id).exists(): # if new tweet
                ...
                aggregateCreated = JetblueAggregateModel.objects.get_or_create(tweet_id=connectionsTweet.id, name=str(connectionsTweet.username), text=str(connectionsTweet.tweet), link=str(connectionsTweet.link), date=connectionsTweet.datestamp, prediction_level=score, category="connections")
        ...
        condensedCreated = JetblueCondensedModel.objects.get_or_create(date=timezone.now(), average_prediction=averageScore, category="connections")