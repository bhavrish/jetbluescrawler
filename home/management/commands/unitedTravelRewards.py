...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        c = twint.Config()
        c.Search = "#unitedairlines travel rewards"
        c.Limit = 5
        c.Store_object = True
        c.Hide_output = True
        twint.run.Search(c)
        ...
        for travelRewardsTweet in travelRewardsTweets[:]:
            if not UnitedAggregateModel.objects.filter(tweet_id=travelRewardsTweet.id).exists(): # if new tweet
                ...
                aggregateCreated = UnitedAggregateModel.objects.get_or_create(tweet_id=travelRewardsTweet.id, name=str(travelRewardsTweet.username), text=str(travelRewardsTweet.tweet), link=str(travelRewardsTweet.link), date=travelRewardsTweet.datestamp, prediction_level=score, category="travel-rewards")
        ...
        condensedCreated = UnitedCondensedModel.objects.get_or_create(date=timezone.now(), average_prediction=averageScore, category="travel-rewards")
