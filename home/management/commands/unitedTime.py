...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for timeTweet in timeTweets[:]:
            if not UnitedAggregateModel.objects.filter(tweet_id=timeTweet.id).exists(): # if new tweet
                ...
                aggregateCreated = UnitedAggregateModel.objects.get_or_create(tweet_id=timeTweet.id, name=str(timeTweet.username), text=str(timeTweet.tweet), link=str(timeTweet.link), date=timeTweet.datestamp, prediction_level=score, category="timeliness")
        ...
        condensedCreated = UnitedCondensedModel.objects.get_or_create(date=timezone.now(), average_prediction=averageScore, category="timeliness")