...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for costTweet in costTweets[:]:
            if not SpiritAggregateModel.objects.filter(tweet_id=costTweet.id).exists(): # if new tweet
                ...
                aggregateCreated = SpiritAggregateModel.objects.get_or_create(tweet_id=costTweet.id, name=str(costTweet.username), text=str(costTweet.tweet), link=str(costTweet.link), date=costTweet.datestamp, prediction_level=score, category="cost")
        ...
        condensedCreated = SpiritCondensedModel.objects.get_or_create(date=timezone.now(), average_prediction=averageScore, category="cost")