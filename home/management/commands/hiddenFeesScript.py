...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for hiddenFeesTweet in hiddenFeesTweets[:]:
            if not JetblueAggregateModel.objects.filter(tweet_id=hiddenFeesTweet.id).exists(): # if new tweet
                ...
                aggregateCreated = JetblueAggregateModel.objects.get_or_create(tweet_id=hiddenFeesTweet.id, name=str(hiddenFeesTweet.username), text=str(hiddenFeesTweet.tweet), link=str(hiddenFeesTweet.link), date=hiddenFeesTweet.datestamp, prediction_level=score, category="hidden-fees")
        ...
        condensedCreated = JetblueCondensedModel.objects.get_or_create(date=timezone.now(), average_prediction=averageScore, category="hidden-fees")