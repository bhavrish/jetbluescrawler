...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        c = twint.Config()
        c.Search = "#unitedairlines reimbursement"
        c.Limit = 5
        c.Store_object = True
        c.Hide_output = True
        twint.run.Search(c)
        ...
        for reimbursementTweet in reimbursementTweets[:]:
            if not UnitedAggregateModel.objects.filter(tweet_id=reimbursementTweet.id).exists(): # if new tweet
                ...
                aggregateCreated = UnitedAggregateModel.objects.get_or_create(tweet_id=reimbursementTweet.id, name=str(reimbursementTweet.username), text=str(reimbursementTweet.tweet), link=str(reimbursementTweet.link), date=reimbursementTweet.datestamp, prediction_level=score, category="reimbursement")
        ...
        condensedCreated = UnitedCondensedModel.objects.get_or_create(date=timezone.now(), average_prediction=averageScore, category="reimbursement")
