...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        c = twint.Config()
        c.Search = "#americanairlines service"
        c.Limit = 5
        c.Store_object = True
        c.Hide_output = True
        twint.run.Search(c)
        ...
        for serviceTweet in serviceTweets[:]:
            if not AmericanAggregateModel.objects.filter(tweet_id=serviceTweet.id).exists(): # if new tweet
                ...
                aggregateCreated = AmericanAggregateModel.objects.get_or_create(tweet_id=serviceTweet.id, name=str(serviceTweet.username), text=str(serviceTweet.tweet), link=str(serviceTweet.link), date=serviceTweet.datestamp, prediction_level=score, category="service")
        ...
        condensedCreated = AmericanCondensedModel.objects.get_or_create(date=timezone.now(), average_prediction=averageScore, category="service")