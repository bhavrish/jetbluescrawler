...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        c = twint.Config()
        c.Search = "#spiritairlines connections"
        c.Limit = 5
        c.Store_object = True
        c.Hide_output = True
        twint.run.Search(c)
        ...
        for connectionsTweet in connectionsTweets[:]:
            if not SpiritAggregateModel.objects.filter(tweet_id=connectionsTweet.id).exists(): # if new tweet
                ...
                aggregateCreated = SpiritAggregateModel.objects.get_or_create(tweet_id=connectionsTweet.id, name=str(connectionsTweet.username), text=str(connectionsTweet.tweet), link=str(connectionsTweet.link), date=connectionsTweet.datestamp, prediction_level=score, category="connections")
        ...
        condensedCreated = SpiritCondensedModel.objects.get_or_create(date=timezone.now(), average_prediction=averageScore, category="connections")