...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        c = twint.Config()
        c.Search = "#jetblue travel rewards"
        c.Limit = 5
        c.Store_object = True
        c.Hide_output = True
        twint.run.Search(c)
        ...
        for travelRewardsTweet in travelRewardsTweets[:]:
            if not JetblueAggregateModel.objects.filter(tweet_id=travelRewardsTweet.id).exists(): # if new tweet
                ...
                aggregateCreated = JetblueAggregateModel.objects.get_or_create(tweet_id=travelRewardsTweet.id, name=str(travelRewardsTweet.username), text=str(travelRewardsTweet.tweet), link=str(travelRewardsTweet.link), date=travelRewardsTweet.datestamp, prediction_level=score, category="travel-rewards")
        ...
        condensedCreated = JetblueCondensedModel.objects.get_or_create(date=timezone.now(), average_prediction=averageScore, category="travel-rewards")