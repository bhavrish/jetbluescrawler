...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        c = twint.Config()
        c.Search = "#americanairlines food entertainment"
        c.Limit = 5
        c.Store_object = True
        c.Hide_output = True
        twint.run.Search(c)
        ...
        for foodEntertainmentTweet in foodEntertainmentTweets[:]:
            if not AmericanAggregateModel.objects.filter(tweet_id=foodEntertainmentTweet.id).exists(): # if new tweet
                ...
                aggregateCreated = AmericanAggregateModel.objects.get_or_create(tweet_id=foodEntertainmentTweet.id, name=str(foodEntertainmentTweet.username), text=str(foodEntertainmentTweet.tweet), link=str(foodEntertainmentTweet.link), date=foodEntertainmentTweet.datestamp, prediction_level=score, category="food-entertainment")
        ...
        condensedCreated = AmericanCondensedModel.objects.get_or_create(date=timezone.now(), average_prediction=averageScore, category="food-entertainment")