...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        c = twint.Config()
        c.Search = "#spiritairlines hidden fees"
        c.Limit = 5
        c.Store_object = True
        c.Hide_output = True
        twint.run.Search(c)
        ...
        for hiddenFeesTweet in hiddenFeesTweets[:]:
            if not SpiritAggregateModel.objects.filter(tweet_id=hiddenFeesTweet.id).exists(): # if new tweet
                ...
                aggregateCreated = SpiritAggregateModel.objects.get_or_create(tweet_id=hiddenFeesTweet.id, name=str(hiddenFeesTweet.username), text=str(hiddenFeesTweet.tweet), link=str(hiddenFeesTweet.link), date=hiddenFeesTweet.datestamp, prediction_level=score, category="hidden-fees")
        ...
        condensedCreated = SpiritCondensedModel.objects.get_or_create(date=timezone.now(), average_prediction=averageScore, category="hidden-fees")