...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        c = twint.Config()
        c.Search = "#spiritairlines legroom"
        c.Limit = 5
        c.Store_object = True
        c.Hide_output = True
        twint.run.Search(c)
        ...
        for legroomTweet in legroomTweets[:]:
            if not SpiritAggregateModel.objects.filter(tweet_id=legroomTweet.id).exists(): # if new tweet
                ...
                aggregateCreated = SpiritAggregateModel.objects.get_or_create(tweet_id=legroomTweet.id, name=str(legroomTweet.username), text=str(legroomTweet.tweet), link=str(legroomTweet.link), date=legroomTweet.datestamp, prediction_level=score, category="legroom")
        ...
        condensedCreated = SpiritCondensedModel.objects.get_or_create(date=timezone.now(), average_prediction=averageScore, category="legroom")
