...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        c = twint.Config()
        c.Search = "#americanairlines hidden fees"
        c.Limit = 5
        c.Store_object = True
        c.Hide_output = True
        twint.run.Search(c)
        ...
        for hiddenFeesTweet in hiddenFeesTweets[:]:
            if not AmericanAggregateModel.objects.filter(tweet_id=hiddenFeesTweet.id).exists(): # if new tweet
                ...
                aggregateCreated = AmericanAggregateModel.objects.get_or_create(tweet_id=hiddenFeesTweet.id, name=str(hiddenFeesTweet.username), text=str(hiddenFeesTweet.tweet), link=str(hiddenFeesTweet.link), date=hiddenFeesTweet.datestamp, prediction_level=score, category="hidden-fees")
        ...
        condensedCreated = AmericanCondensedModel.objects.get_or_create(date=timezone.now(), average_prediction=averageScore, category="hidden-fees")