...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        c = twint.Config()
        c.Search = "#americanairlines available"
        c.Limit = 5
        c.Store_object = True
        c.Hide_output = True
        twint.run.Search(c)
        ...
        for availableTweet in availableTweets[:]:
            if not AmericanAggregateModel.objects.filter(tweet_id=availableTweet.id).exists(): # if new tweet
                ...
                aggregateCreated = AmericanAggregateModel.objects.get_or_create(tweet_id=availableTweet.id, name=str(availableTweet.username), text=str(availableTweet.tweet), link=str(availableTweet.link), date=availableTweet.datestamp, prediction_level=score, category="availability")
        ...
        condensedCreated = AmericanCondensedModel.objects.get_or_create(date=timezone.now(), average_prediction=averageScore, category="availability")