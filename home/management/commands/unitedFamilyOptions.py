...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        c = twint.Config()
        c.Search = "#unitedairlines family options"
        c.Limit = 5
        c.Store_object = True
        c.Hide_output = True
        twint.run.Search(c)
        ...
        for familyOptionsTweet in familyOptionsTweets[:]:
            if not UnitedAggregateModel.objects.filter(tweet_id=familyOptionsTweet.id).exists(): # if new tweet
                ...
                aggregateCreated = UnitedAggregateModel.objects.get_or_create(tweet_id=familyOptionsTweet.id, name=str(familyOptionsTweet.username), text=str(familyOptionsTweet.tweet), link=str(familyOptionsTweet.link), date=familyOptionsTweet.datestamp, prediction_level=score, category="family-options")
        ...
        condensedCreated = UnitedCondensedModel.objects.get_or_create(date=timezone.now(), average_prediction=averageScore, category="family-options")
