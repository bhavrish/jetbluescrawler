...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for costTweet in costTweets[:]:
            if not JetblueAggregateModel.objects.filter(tweet_id=costTweet.id).exists(): # if new tweet
                ...
                aggregateCreated = JetblueAggregateModel.objects.get_or_create(tweet_id=costTweet.id, name=str(costTweet.username), text=str(costTweet.tweet), link=str(costTweet.link), date=costTweet.datestamp, prediction_level=score, category="cost")
        ...
        condensedCreated = JetblueCondensedModel.objects.get_or_create(date=timezone.now(), average_prediction=averageScore, category="cost")